...
const GENERATE_TEMPLATES = {
  ...
  "cost-efficient-sonnet": "generate-cost-efficient-sonnet.txt",
...
export async function buildGeneratePrompt({
  ...
  const isSonnet = typeof targetModel === "string" && targetModel.toLowerCase().includes("sonnet");
  const templateKey =
    optimizationPath === "cost-efficient" && isSonnet ? "cost-efficient-sonnet" : optimizationPath;
  const templateName = GENERATE_TEMPLATES[templateKey] || GENERATE_TEMPLATES["cost-efficient"];
