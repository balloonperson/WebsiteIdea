...
/**
 * Same call as callAi, but also returns token usage so callers (the
 * evolution orchestrator) can record real input/output token counts and
 * derive cost instead of guessing.
 */
export async function callAiWithUsage({ system, user }) {
  if (!process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_API_KEY.includes("your_")) {
    const error = new Error("Missing Anthropic API key. Add ANTHROPIC_API_KEY to your .env file.");
    error.statusCode = 400;
    throw error;
